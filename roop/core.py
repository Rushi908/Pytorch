...
def pre_check() -> bool:
    ...
    util.conditional_download(download_directory_path, ['https://huggingface.co/Rookiehan/facefusion/resolve/main/inswapper_128_fp16.onnx'])
...
def batch_process(output_method, files:list[ProcessEntry], use_new_method) -> None:
    ...
    for index, f in enumerate(files):
        ...
        if util.has_image_extension(fullname):
            ...
            destination = util.get_destfilename_from_path(fullname, roop.globals.output_path, f'_process.{roop.globals.output_image_format}')
            # destination = util.replace_template(destination, index=index)
            destination = destination.replace("_process", "Man")
            ...
            destination = util.get_destfilename_from_path(fullname, roop.globals.output_path, f'_process.{roop.globals.output_video_format}')
            f.finalname = destination
            videofiles.append(f)
        ...
        for index,v in enumerate(videofiles):
            ...
            if os.path.isfile(video_file_name):
                ...
                if util.has_extension(v.filename, ['gif']):
                    ...
                    destination = v.finalname.replace("_process", "Man")
                    pathlib.Path(os.path.dirname(destination)).mkdir(parents=True, exist_ok=True)
...
def run() -> None:
    ...
    print(f"CFG File (config.yaml): {roop.globals.CFG}")
    print(f"Max Threads: {roop.globals.CFG.max_threads}")
    print(f"Output Video Codec: {roop.globals.CFG.output_video_codec}")
    print(f"CUDA Device ID: {roop.globals.cuda_device_id}")
    print(f"Execution Threads: {roop.globals.execution_threads}")
    print(f"Video Encoder: {roop.globals.video_encoder}")
    main.run()

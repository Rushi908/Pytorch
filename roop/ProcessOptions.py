...
class ProcessOptions:
    def __init__(self, swap_model, processordefines: dict, face_distance, blend_ratio, swap_mode, selected_index, masking_text, imagemask, num_steps, subsample_size, show_face_area, restore_original_mouth, show_mask=False):
        if swap_model is not None:
            ...
            # Extract numeric part from the last segment of swap_model
            size_match = re.search(r'\d+', swap_model.split()[-1])
            if size_match:
                self.swap_output_size = int(size_match.group())
            else:
                # Default to 128 if no valid number is found
                print(f"Warning: Unable to extract size from swap_model '{swap_model}', defaulting to 128.")
                self.swap_output_size = 128
        else:
            self.swap_output_size = 128  # Default value if swap_model is None

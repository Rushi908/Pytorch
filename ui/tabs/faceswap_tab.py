...
print(list_files_process, "This is list files process")
...
def faceswap_tab():
        ...
        with gr.Row(variant='panel'):
            ...
            with gr.Column(scale=2):
                ...
                maskimage = gr.ImageEditor(label="Manual mask Image", sources=["clipboard"], transforms="", type="numpy",
                                             brush=gr.Brush(color_mode="fixed", colors=["rgba(255, 255, 255, 1"]), interactive=True, visible=True)
                
                print(maskimage, "This is mask photo by faceswap tab function by faceswap tab file.")
    ...
    print(previewinputs, "This is preview input variable.")
    
    previewoutputs = [previewimage, maskimage, preview_frame_num]

    print(previewoutputs, "This is preview output variable.") 
    ...
    main_data = [ui.globals.ui_selected_swap_model, output_method, ui.globals.ui_selected_enhancer, selected_face_detection, roop.globals.keep_frames, roop.globals.wait_after_extraction,
    ...
    print(main_data, "This is main data variable by faceswap tab function for faceswap tab file.")
    ...
    print(start_event, "This is start event by faceswap tab function.")
    
        
    after_swap_event = start_event.success(fn=on_resultfiles_finished, inputs=[resultfiles], outputs=[resultimage, resultvideo])

    print(after_swap_event, "This is event complete message by swap event by faceswap tab function.")
...
def on_srcfile_changed(srcfiles, progress=gr.Progress()):
    ...
    for f in srcfiles:    
        source_path = f.name
        print(source_path, "This is source path by srcfile changed function by faceswap tab file.")
        # print(roop.globals.INPUT_FACESETS, "This is facesets by source file function function by faceswap tab file.")
        if source_path.lower().endswith('fsz'):
            ...
            print("This is all faces data by source file by source function by faceswap tab file.")
            ...
            if len(face_set.faces) > 0:
                ...
                # print(roop.globals.INPUT_FACESETS[0].face, "This is facesets by source file function by faceswap tab file.")
            ...
            for f in SELECTION_FACES_DATA:
                ...
                ui.globals.ui_input_thumbs.append(image)
                roop.globals.INPUT_FACESETS.append(face_set)
                print(roop.globals.INPUT_FACESETS[0].faces, "This is facesets by source file function by faceswap tab file.")
...
def on_use_face_from_selected(files, frame_num):
    ...
    print(files[selected_preview_index].name, "This is target path by face from selected function by faceswap tab file.")
...
def on_selected_face():
    ...
    if IS_INPUT:
        ...
        print(roop.globals.INPUT_FACESETS[0].faces, "This is facesets by seletced function function by faceswap tab file.")
        roop.globals.INPUT_FACESETS.append(face_set)
        ui.globals.ui_input_thumbs.append(image)
...
def on_preview_frame_changed(swap_model, frame_num, files, fake_preview, enhancer, detection, face_distance, blend_ratio,
    ...
    print(SELECTED_INPUT_FACE_INDEX, "This is selected by frame function by faceswap tab file.")
    ...
    if len(roop.globals.INPUT_FACESETS) > SELECTED_INPUT_FACE_INDEX:
        ...
        # print(mask_offsets, "This is mask variable for frame function by faceswap tab file.")
        # print(roop.globals.INPUT_FACESETS[0].faces, "This is facesets by frame function by faceswap tab file.")
...
def on_toggle_masking(previewimage, mask):
    ...
    print(previewimage, "This is previewimage by toggle mask for faceswap tab file.")
    
    if manual_masking:
        layers = mask["layers"]
        if len(layers) == 1:
            layers = [create_blank_image(previewimage.shape[1],previewimage.shape[0])]
        return gr.Image(visible=False), gr.ImageEditor(value={"main_data": previewimage, "layers": layers, "composite": None}, visible=True)
...
def index_of_no_face_action(dropdown_text):
    ...
    print(no_face_choices.index(dropdown_text), "This is face action by faceswap tab file.")
...
def is_sparse(matrix):
    ...
    # Check if there are any non-zero elements in the matrix
...
def start_swap( swap_model, output_method, enhancer, detection, keep_frames, wait_after_extraction, skip_audio, face_distance, blend_ratio,
    ...
    # print(swap_model, output_method, enhancer, detection, keep_frames, wait_after_extraction, skip_audio, face_distance, blend_ratio, selected_mask_engine, clip_text, processing_method, no_face_action, vr_mode, autorotate, restore_original_mouth, num_swap_steps, upsample, imagemask, "This is start swap arguments by faceswap tab file.")

    '''print(swap_model, type(swap_model), "This is swap model by start swap function by faceswap tab file.")
    print(output_method, type(output_method), "This is output method by start swap function by faceswap tab file.")
    print(enhancer, type(enhancer), "This is enhancer by start swap function by faceswap tab file.")'''

    
    '''print(swap_model, type(swap_model), "This is swap model by start swap function by faceswap tab file.")
    print(output_method, type(output_method), "This is output method by start swap function by faceswap tab file.")
    print(enhancer, type(enhancer), "This is enhancer by start swap function by faceswap tab file.")
    print(detection, type(detection), "This is detection by start swap function by faceswap tab file.")
    print(keep_frames, type(keep_frames), "This is keep_frames by start swap function by faceswap tab file.")
    print(wait_after_extraction, type(wait_after_extraction), "This is wait_after_extraction by start swap function by faceswap tab file.")
    print(skip_audio, type(skip_audio), "This is skip_audio by start swap function by faceswap tab file.")
    print(face_distance, type(face_distance), "This is face_distance by start swap function by faceswap tab file.")
    print(blend_ratio, type(blend_ratio), "This is blend_ratio by start swap function by faceswap tab file.")
    print(selected_mask_engine, type(selected_mask_engine), "This is selected_mask_engine by start swap function by faceswap tab file.")
    print(clip_text, type(clip_text), "This is clip_text by start swap function by faceswap tab file.")
    print(processing_method, type(processing_method), "This is processing_method by start swap function by faceswap tab file.")
    print(no_face_action, type(no_face_action), "This is no_face_action by start swap function by faceswap tab file.")
    print(vr_mode, type(vr_mode), "This is vr_mode by start swap function by faceswap tab file.")
    print(autorotate, type(autorotate), "This is autorotate by start swap function by faceswap tab file.")
    print(restore_original_mouth, type(restore_original_mouth), "This is restore_original_mouth by start swap function by faceswap tab file.")
    print(num_swap_steps, type(num_swap_steps), "This is num_swap_steps by start swap function by faceswap tab file.")
    print(upsample, type(upsample), "This is upsample by start swap function by faceswap tab file.")
    print(imagemask, type(imagemask), "This is imagemask main data by start swap function by faceswap tab file.")
    print(imagemask["layers"][0].shape, type(imagemask), "This is imagemask layers by start swap function by faceswap tab file.")
    print(imagemask["composite"].shape, type(imagemask), "This is imagemask composite data by start swap function by faceswap tab file.")
    print("This is start swap arguments by faceswap tab file.")'''
    # print(roop.globals.INPUT_FACESETS[0].faces, "This is facesets by start swap function by faceswap tab file.")

    main = is_sparse(imagemask["layers"][0])

    if not main:
        print("This is not sparse matrix type.")

    else:
        print("This is sparse matrix type.")
    ...
    # print(mask_engine, "This is mask engine by start swap function.")
    ...
    roop.globals.execution_threads = roop.globals.CFG.max_threads
    # print(roop.globals.CFG.max_threads, "This is max threads easy by start swap function by faceswap tab file.")
    
    roop.globals.video_encoder = roop.globals.CFG.output_video_codec
    # print(roop.globals.CFG.output_video_codec, "This is output video codec by start swap function by faceswap tab file.")

    roop.globals.video_quality = roop.globals.CFG.video_quality
    # print(roop.globals.CFG.video_quality, "This is video quality by start swap function by faceswap tab file.")

    roop.globals.max_memory = roop.globals.CFG.memory_limit if roop.globals.CFG.memory_limit > 0 else None
    # print(roop.globals.max_memory, "This is max memory   by sart swap function by faceswap tab file.")

    # print(list_files_process[0].filename, "This is list files process filename by start swap function by faceswap tab file.")
    # print(list_files_process[0].start, "This is list files process start frame by start swap function by faceswap tab file.")
    # print(list_files_process[0].end, "This is list files process end frame by start swap function by faceswap tab file.")
    '''print(list_files_process[0].fps, "This is list files process f.p.s. data by start swap function by faceswap tab file.")
    print(list_files_process[0].finalname, "This is list files process finalname by start swap function by faceswap tab file.")
    print(list_files_process[0].__dict__, "This is list files process all data by start swap function by faceswap tab file.")


    print(roop.globals.source_path, "This is source path by start swap function by faceswap tab file")
    print(roop.globals.target_path, "This is target path by start swap function by faceswap tab file")
    print(roop.globals.output_path, "This is output path by start swap function by faceswap tab file")'''
